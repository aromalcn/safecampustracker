...
const StudentChat = () => {
    ...
    const fetchContactsAndMetadata = async (studentId) => {
        try {
            setLoading(true);
            
            // 1. Fetch Teachers and Admins
            ...
            metaData.forEach(msg => {
                const otherParty = msg.sender_id === studentId ? msg.receiver_id : msg.sender_id;
                
                if (!lastMsgMap[otherParty]) {
                    lastMsgMap[otherParty] = msg.created_at;
                }

                if (msg.receiver_id === studentId && !msg.is_read) {
    ...
    return (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#f3f4f6' }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '1.5rem', maxWidth: '1200px', margin: '0 auto', width: '100%', overflow: 'hidden' }}>
                
                {/* Inline Header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                    <button onClick={() => navigate('/student')} style={{ border: 'none', background: 'white', cursor: 'pointer', padding: '8px', borderRadius: '50%', display: 'flex', boxShadow: '0 1px 2px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
                        <ArrowLeft size={18} color="#64748b" />
                    </button>
                    <h1 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, color: '#1e293b' }}>Messages</h1>
                </div>

                <div style={{ flex: 1, display: 'flex', gap: '2rem', overflow: 'hidden', minHeight: 0 }}>
                ...
                <div style={{ width: '300px', background: 'white', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '1.5rem', borderBottom: '1px solid #f3f4f6' }}>
                        <h2 style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0, color: '#111827' }}>Teachers & Staff</h2>
                    </div>
                ...
                <div style={{ flex: 1, background: 'white', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    {!selectedContact ? (
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <p>Select a teacher or admin to start messaging</p>
                        </div>
                    ) : (
                        <>
                            {/* Chat Header */}
